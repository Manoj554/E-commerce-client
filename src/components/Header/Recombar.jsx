...
const SubCategory = ({ id, name, child, handleClick, i, handleClick2 }) => {
	return (
		<>
			...
			<div className={styles.maindiv}>
				...
				{loading ? <p>Loading categories ...</p> :
					categories.length > 0 && categories.map((val, i) => (
						<SubCategory key={val.id} id={val.id} name={val.name} child={val.children} handleClick={handleClick} handleClick2={handleClick2} i={i} />
					))}
			</div>
		</>
	)
